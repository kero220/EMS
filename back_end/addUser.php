        ...
        <div class="content w-full">
            ...
            <section>
                <div id="container" class="m-auto mt-20 rounded-xl bg-gray-100 p-6 shadow-lg max-w-xl">
                    ...
                    <div id="wrapper">
                        <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post" enctype="multipart/form-data" class="flex flex-col gap-4 h-full max-h-auto min-w-auto">
                            <!-- Image Upload -->
                            <div class="rounded-md border  bg-white p-4 shadow-md w-36 mx-auto">
                                ...
                                <input id="upload" name="file" type="file" class="hidden" />
                                <?php
                            ...
                            <div id="nameDiv" class="flex justify-around gap-4">
                                <input type="text" id="User_name" name="username" placeholder="User Name"
                                    class="rounded-lg text-center font-bold p-2 w-full" required />
                            ...
                            <div id="nameDiv" class="flex justify-around gap-4">
                                ...
                                <input type="number" id="dept_id" name="dept_id" placeholder="Department ID"
                                    class="rounded-lg text-center font-bold p-2 w-full" required />
                            </div>
                            <!-- Branch Location -->
                            <div id="positionDiv">
                            ...
                            if (isset($_POST['submit'])) {
                                ...
                                if (
                                    isset($GLOBALS['image_tmp_name']) && isset($_POST['fname']) &&
                                    isset($_POST['lname'])  &&
                                    isset($_POST['position']) && isset($_POST['manager_id']) &&
                                    isset($_POST['dept_id']) && isset($_POST['branch_location']) && isset($_POST['phone']) &&
                                    isset($_POST['email'])
                                ) {
                                    if (empty($_POST['Cdate'])) {

                                        $new_emp = insert_new_employee(
                                            file_get_contents($GLOBALS['image_tmp_name']),
                                            $_POST['fname'],
                                            $_POST['lname'],
                                            $_POST['date'],
                                            isset($_POST['manager_id']) ? $_POST['manager_id'] : null,
                                            $_POST['dept_id'],
                                            $_POST['position'],
                                            $_POST['branch_location'],
                                            $_POST['email'],
                                            isset($_SESSION['active']) ? $_SESSION['active'] : null,
                                            $_POST['phone'],
                                            $pdo
                                        );
                                        $GLOBALS['new_emp_id'] = $new_emp;

                                        $unique = generate_unique_password_username($GLOBALS['new_emp_id'], $_POST['fname'], $_POST['date'], $pdo);
                                        $GLOBALS['unique'] = $unique;
                                    } else {
                                        $new_emp = insert_new_employee(
                                            file_get_contents($GLOBALS['image_tmp_name']),
                                            $_POST['fname'],
                                        ...
                                        $GLOBALS['new_emp_id'] = $new_emp;
                                        $cur_date = getdate(date('U')); // find current date  YYYY-MM-DD
                                        $str = $cur_date['year'] . "-" . $cur_date['mon'] . "-" . $cur_date['mday'];
                                        $unique = generate_unique_password_username($GLOBALS['new_emp_id'], $_POST['fname'], $str, $pdo);
                                        $GLOBALS['unique'] = $unique;
                                    }

                                    echo "<div class='success'>";
                                    echo "Employee Added Successfully" . "<br>";
                                    echo "Username:" . " " . $GLOBALS['unique'][0] . " " . "Password:" . " " . $GLOBALS['unique'][1];
                                    echo "</div>";
                                } else {
